...
import { getLikedNotifications, type PostNotification } from "@/lib/notifications";
import { getUserPosts, type Post } from "@/lib/posts";

export default function LikedNotificationsPage() {
	const [notifications, setNotifications] = useState<PostNotification[]>([]);
	const [posts, setPosts] = useState<Post[]>([]);

	useEffect(() => {
		const loadNotifications = window.setTimeout(() => {
			setNotifications(getLikedNotifications().sort((a, b) => b.id - a.id));
		...
		<main className="site-shell mypage-shell">
			...
			<section className="mypage-content">
				...
				{visibleNotifications.length === 0 ? (
					<div className="empty-posts"><div className="empty-mark">♡</div><h3>まだ通知はありません</h3><p>あなたの投稿にいいねが付くと、ここに表示されます。</p></div>
				) : (
					<ul className="notification-list">
						{visibleNotifications.map((notification) => {
							const post = ownPosts.get(notification.postId);
							if (!post) return null;
							return <li className="notification-item" key={notification.id}><span className="notification-mark">♡</span><div><p><strong>{notification.actorName}</strong> が「{post.title || "お気に入りの購入品"}」にいいねしました。</p><Link href={`/posts/${post.id}`}>投稿を見る</Link></div></li>;
						})}
					</ul>
				)}
			</section>
